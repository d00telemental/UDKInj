...
FStringBase<WithRAII>::AppendFormatv(const_pointer const Format, std::va_list Args) {
    ...
    UGHSDK_CHECK(FormatCount >= 0, "invalid _vscwprintf format");
    ...
    UGHSDK_CHECK(CheckCount == FormatCount, "unknown _vsnwprintf failure");
...
FStringBase<WithRAII>& FStringBase<WithRAII>::AppendAnsi(char const* const InAnsiStr) {
    UGHSDK_CHECK(InAnsiStr != nullptr, "");
    if (InAnsiStr != nullptr) {
        auto const InAnsiLength = static_cast<size_type>(std::strlen(InAnsiStr));
        auto const OutWideLength = UGHSDK::GetWideLengthAnsi(InAnsiStr, InAnsiLength);

        size_type const NewLength = this->Length() + OutWideLength;
        Storage.Reserve(NewLength + 1);

        DWORD WinError = 0u;
        bool const bEncodeOkay = UGHSDK::EncodeWideFromAnsi(InAnsiStr, InAnsiLength,
                Storage.GetData() + this->Length(), OutWideLength, &WinError);
        UGHSDK_CHECK(bEncodeOkay, "failure encoding ansi as wide string");
...
FStringBase<WithRAII>& FStringBase<WithRAII>::AppendUtf8(char const* const InUtf8Str) {
    UGHSDK_CHECK(InUtf8Str != nullptr, "");
    if (InUtf8Str != nullptr) {
        auto const InUtf8Length = static_cast<size_type>(std::strlen(InUtf8Str));
        auto const OutWideLength = UGHSDK::GetWideLengthUtf8(InUtf8Str, InUtf8Length);

        size_type const NewLength = this->Length() + OutWideLength;
        Storage.Reserve(NewLength + 1);

        DWORD WinError = 0u;
        bool const bEncodeOkay = UGHSDK::EncodeWideFromUtf8(InUtf8Str, InUtf8Length,
            Storage.GetData() + this->Length(), OutWideLength, &WinError);
        UGHSDK_CHECK(bEncodeOkay, "failure encoding utf-8 as wide string");
